...
public class ProjectEscalationController {
    ...
    public ResponseEntity<ApiResponse<ProjectEscalationDTO>> addEscalationContact(@RequestBody @Valid ProjectEscalationDTO dto) {
        ProjectEscalationDTO created = projectEscalationService.addEscalationContact(dto);
        return new ResponseEntity<>(new ApiResponse<>(true, "Escalation contact added successfully", created), HttpStatus.CREATED);
    }
