...
public class DashboardKpiServiceImpl implements DashboardKpiService {
    ...
    @Override
    @Cacheable(value = "dashboard-kpis", key = "(#fromDate != null ? #fromDate : T(java.time.LocalDate).now()) + '-' + (#toDate != null ? #toDate : T(java.time.LocalDate).now()) + '-' + (#role ?: 'ALL') + '-' + (#location ?: 'ALL') + '-' + (#employmentType ?: 'ALL') + '-' + (#minExperience ?: '0') + '-' + (#maxExperience ?: '999')")
